...
| ![image](./docs/bag_item.png) | <center> **Bag Item** <br/> Adds in a "bag" item that can hold mass quantities of a small number of different items; Designed to complement vein-mine. Items collected will automatically go into the bag if that item type is stored, including when the player's inventory is full. Higher tiers can be made to store more items and item types. </center> | `enableBagItem: boolean = true` |
