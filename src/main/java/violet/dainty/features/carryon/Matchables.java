...
public final class Matchables {
	...
	private static Optional<Float> getValueFromString(String toGetFrom, String key) {
		if (toGetFrom == null || toGetFrom.isEmpty()) return Optional.empty();
	...
	public record NumberBoundCondition(String bounds) implements Matchable<Number> {
		...
		public boolean matches(Number num) {
			...
			try {
				if (bounds.contains("<=")) return number <= Double.parseDouble(bounds.replace("<=", ""));
				if (bounds.contains(">=")) return number >= Double.parseDouble(bounds.replace(">=", ""));
				if (bounds.contains("<")) return number < Double.parseDouble(bounds.replace("<", ""));
				if (bounds.contains(">")) return number > Double.parseDouble(bounds.replace(">", ""));
				if (bounds.contains("=")) return number == Double.parseDouble(bounds.replace("=", ""));
				else return number == Double.parseDouble(bounds);
			}
			catch (Exception e) {
	...
	public record GamestageCondition(String gamestage) implements Matchable<ServerPlayer> {
		...
		public boolean matches(ServerPlayer player) {
			...
			if (gamestage == null || gamestage.isEmpty()) return true;
			return hasStage(player, gamestage);
		}
	}
	...
	public record ScoreboardCondition(String cond) implements Matchable<ServerPlayer> {
		...
		public boolean matches(ServerPlayer player) {
			...
			if (iG == -1 || (iE < iG && iL == -1 || iE < iL && iE != -1)) numb = cond.substring(iE);
			else if (iE == -1 || (iG < iE && iL == -1 || iG < iL && iG != -1)) numb = cond.substring(iG);
			else numb = cond.substring(iL);
