...
public class ExplorersCompassClient {
	...
	public static void clientInit(FMLClientSetupEvent event) {
		event.enqueueWork(() -> {
			ItemProperties.register(StructureCompass.explorersCompass, ResourceLocation.fromNamespaceAndPath(Dainty.MODID, "angle"), new ClampedItemPropertyFunction() {
				@OnlyIn(Dist.CLIENT)
				...
				private double getAngle(ClientLevel world, Entity entity, ItemStack stack) {
					if (stack.getItem() == StructureCompass.explorersCompass) {
    ...
    public static void registerOverlay(RegisterGuiLayersEvent event) {
        event.registerAbove(VanillaGuiLayers.BOSS_OVERLAY, ResourceLocation.fromNamespaceAndPath(Dainty.MODID, "explorers_compass"), new ExplorersCompassOverlay());
    }
