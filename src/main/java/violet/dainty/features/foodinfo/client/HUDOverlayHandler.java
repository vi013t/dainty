...
import violet.dainty.features.foodinfo.api.event.HUDOverlayEvent;
import violet.dainty.features.foodinfo.helpers.FoodHelper;
import violet.dainty.features.foodinfo.helpers.HungerHelper;
import violet.dainty.features.foodinfo.helpers.TextureHelper;
import violet.dainty.features.foodinfo.util.IntPoint;
...
public class HUDOverlayHandler
{
	...
	public static void register(RegisterGuiLayersEvent event)
	{
		...
		event.registerBelow(
			...
			ResourceLocation.fromNamespaceAndPath(Dainty.MODID, "health_offset"),
			(guiGraphics, deltaTracker) -> healthIconsOffset = Minecraft.getInstance().gui.leftHeight
		);
		event.registerBelow(
			VanillaGuiLayers.FOOD_LEVEL,
			ResourceLocation.fromNamespaceAndPath(Dainty.MODID, "food_offset"),
	...
	public static class HealthOverlay extends Overlay {
		public static final ResourceLocation ID = ResourceLocation.fromNamespaceAndPath(Dainty.MODID, "health_restored");
	...
	{
		public static final ResourceLocation ID = ResourceLocation.fromNamespaceAndPath(Dainty.MODID, "hunger_restored");
		...
		public void render(Minecraft mc, Player player, GuiGraphics guiGraphics, int left, int right, int top, int guiTicks)
		{
			...
			modifiedFoodProperties.saturation();
		...
		public boolean shouldRenderOverlay(Minecraft mc, Player player, GuiGraphics guiGraphics, int guiTicks)
		{
			...
			return ModConfig.SHOW_FOOD_VALUES_OVERLAY;
		}
	}

	public static class SaturationOverlay extends Overlay
	{
		public static final ResourceLocation ID = ResourceLocation.fromNamespaceAndPath(Dainty.MODID, "saturation_level");
		...
		public boolean shouldRenderOverlay(Minecraft mc, Player player, GuiGraphics guiGraphics, int guiTicks)
		{
			...
			return ModConfig.SHOW_SATURATION_OVERLAY;
		}
	}

	public static class ExhaustionOverlay extends Overlay
	{
		public static final ResourceLocation ID = ResourceLocation.fromNamespaceAndPath(Dainty.MODID, "exhaustion_level");
	...
	public static void onClientTick(ClientTickEvent.Post event)
	{
		...
		flashAlpha = Math.max(0F, Math.min(1F, unclampedFlashAlpha)) * Math.max(0F, Math.min(1F, (float) ModConfig.MAX_HUD_OVERLAY_FLASH_ALPHA));
	}
	...
	private static boolean shouldShowEstimatedHealth(Player player)
	{
		...
		if (!ModConfig.SHOW_FOOD_HEALTH_HUD_OVERLAY)
			return false;
	...
	private static class OffsetsCache
	{
		...
		protected void generate(int guiTicks, Player player)
		{
			...
			if (ModConfig.SHOW_VANILLA_ANIMATION_OVERLAY)
			{
				FoodData stats = player.getFoodData();
	...
	public static class HeldFoodCache
	{
		...
		protected void query(Player player)
		{
			...
			if (ModConfig.SHOW_FOOD_VALUES_OVERLAY_WHEN_OFFHAND && !canConsume)
			{
