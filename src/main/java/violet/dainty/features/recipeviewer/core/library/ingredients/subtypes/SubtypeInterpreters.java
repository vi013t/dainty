...
public class SubtypeInterpreters {
	...
	public <B, I> boolean addInterpreter(IIngredientTypeWithSubtypes<B, I> type, B base, violet.dainty.features.recipeviewer.core.commonapi.ingredients.subtypes.IIngredientSubtypeInterpreter<I> interpreter) {
		if (!type.getIngredientBaseClass().isInstance(base)) {
			throw new IllegalArgumentException(
				String.format("base must be instance of %s but got %s instead", type.getIngredientBaseClass(), base.getClass())
			);
