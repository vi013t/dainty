	...
	private record RecipeLayoutUserInputHandler<R>(IRecipeLayoutDrawable<R> recipeLayout) implements IUserInputHandler {
		...
		private boolean handleCopyRecipeId(IRecipeLayoutDrawable<R> recipeLayout, boolean simulate) {
			...
			if (registryName == null) {
				MutableComponent message = Component.translatable("dainty.message.copy.recipe.id.failure");
				if (player != null) {
			...
			MutableComponent message = Component.translatable("dainty.message.copy.recipe.id.success", Component.literal(recipeId));
			if (player != null) {
				player.displayClientMessage(message, false);
			}
			return true;
