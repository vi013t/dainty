...
public class EntityDetailsBodyProvider implements IEntityComponentProvider {
	...
	public void appendTooltip(ITooltip tooltip, EntityAccessor accessor, IPluginConfig config) {
		...
		if (poseId != Pose.STANDING.id()) {
			String key = "dainty.access.entity.pose.%s".formatted(poseId);
			if (I18n.exists(key)) {
				tooltip.add(Component.translatable("dainty.access.entity.pose", Component.translatable(key)));
			}
		}
		if (entity instanceof Leashable leashable && leashable.isLeashed()) {
			...
			if (holder instanceof LeashFenceKnotEntity knot) {
				...
				tooltip.add(Component.translatable("dainty.access.entity.leashed_to", blockState.getBlock().getName()));
			} else if (holder != null) {
				tooltip.add(Component.translatable("dainty.access.entity.leashed_to", holder.getName()));
			}
		}
	}
