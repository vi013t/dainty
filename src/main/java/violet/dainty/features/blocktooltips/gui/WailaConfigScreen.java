...
import violet.dainty.features.blocktooltips.gui.config.OptionButton;
...
public class WailaConfigScreen extends PreviewOptionsScreen {
	...
	public static OptionsList.Entry editIgnoreList(OptionsList.Entry entry, String fileName, Runnable defaultFactory) {
		...
		MutableComponent tooltip = Component.translatable("config.dainty.edit_ignore_list");
	...
	public OptionsList createOptions() {
		...
		if (!ConfigGeneral.itemModNameTooltipDisabledByMods.isEmpty()) {
			...
			value.appendDescription(Component.translatable("gui.dainty.disabled_by_mods"));
		...
		options.choices(
				...
				id -> Component.translatable(Util.makeDescriptionId("dainty.theme", id)));
